...
import { fetchInvoicesByClass } from '../../../api/api';
...
const Invoice = ({classId}: {classId: string}) => {
     ...
     const getStatusColor = (status: Status) => {
      ...
      const deleteInvoiceHandler = async (invoiceId: string) => {
        ...
        if(response.success){
          ...
          const data = invoices.filter((invoice) => invoice._id !== response.data._id)
          setInvoices(data)
        }
      }
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center my-5 gap-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
        <button
          onClick={() => navigate(`/dashboard/classes/${classId}/invoice/new`)}
                    ...
                    <tr key={2} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {invoice.status == "Unpaid" && (
                                  <button onClick={() => deleteInvoiceHandler(invoice._id)} className="bg-gray-500 text-white text-sm px-3 py-1 rounded hover:bg-gray-600">
                                  Delete
                                </button>
                        )}
