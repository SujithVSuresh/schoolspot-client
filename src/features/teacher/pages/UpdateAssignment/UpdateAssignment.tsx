...
import { fetchAssignmentById, updateAssignment } from "../../api/api";
...
const UpdateAssignment = () => {
  ...
  const assignmentlId = location.pathname.split("/")[5]
  ...
  useEffect(() => {
    const fetchAssignmentHandler = async (assignmentlId: string) => {
        const response = await fetchAssignmentById(assignmentlId)
        if(response.success){

            const assignment = response.data.data

            const formattedDueDate = formatLocalDateTime(assignment.dueDate);

            setValue("title", assignment.title)
            setValue("description", assignment.description)
            setValue("submissionType", assignment.submissionType)
            setValue("dueDate", formattedDueDate)
        }
    }

    fetchAssignmentHandler(assignmentlId)
  ...
  const onSubmit = async (data: AddAssignmentType) => {
    const response = await updateAssignment(data, assignmentlId)

    if(response.success){
      navigate(`/teacher/classes/${classId}/assignments/${assignmentlId}`)
    }
  }
